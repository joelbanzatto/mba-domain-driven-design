...
export const WaitingListSchema = new EntitySchema<WaitingList>({
  ...
  properties: {
    ...
    event_id: { customType: new EventIdSchemaType() },
    section_id: { customType: new EventSectionIdSchemaType() },
    entries: {
      reference: '1:m',
...
export const WaitingListEntrySchema = new EntitySchema<WaitingListEntry>({
  ...
  properties: {
    ...
    customer_id: { customType: new CustomerIdSchemaType() },
    status: { type: 'string', length: 20 },
    created_at: { type: 'datetime', length: 3 },
    waiting_list_id: {
      reference: 'm:1',
      entity: () => WaitingList,
      hidden: true,
